...
# Self-contained data generator
def generate_transaction_data(n_transactions=10000):
    """Generate realistic transaction data for fraud detection training"""
    np.random.seed(42)
    ...
    risk_score = (
        ...
        (df['account_age_days'] < 30).astype(int) * 0.25 +
        (df['transaction_hour'].isin([2, 3, 4])).astype(int) * 0.2 +
        (df['days_since_last_transaction'] > 30).astype(int) * 0.15 +
        df['merchant_risk_score'] * 0.4 +
        df['transaction_country_risk'] * 0.3 +
        df['payment_method_risk'] * 0.2
    )
    
    # Add some randomness and create fraud labels
